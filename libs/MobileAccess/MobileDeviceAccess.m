...
- (NSArray *)browse:(NSString*)type
{
    ...
    if(type){
        pre=[NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
            return [[((AMApplication*)evaluatedObject).info objectForKey:@"ApplicationType"] isEqual:type];
        }];
    }
	return [self browseFiltered:pre];
}
...
void AMNotificationProxy_callback(CFStringRef notification, void* data)
{
    ...
    NSLog(@"recive notification:%@",notification);
	if (proxy->_messages) {
	...
	if ([self deviceConnect]) {
		if ([self startSession]) {
			...
			if (
				...
				NSDictionary *info = [(NSDictionary*)dict objectForKey:bundleId];
				if (info) {
